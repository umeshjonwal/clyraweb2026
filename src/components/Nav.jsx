    ...
    >
      <div className={`max-w-6xl mx-auto flex items-center justify-between p-2 rounded-full border transition-all duration-500 ${
        ...
        <div className="flex items-center space-x-2">
          <button onClick={toggleTheme} className={`p-3 rounded-full transition-all border ${
        ...
        {open && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            <div className="flex flex-col space-y-6 text-center">
              {navLinks.map((link) => (
                <a key={link.label} href={link.href} className={`text-2xl font-black uppercase tracking-tighter ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{link.label}</a>
              ))}
              <a href="/contact" className="bg-blue-600 text-white py-5 rounded-2xl font-black uppercase text-xs tracking-widest">Let's Talk</a>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
